...
_SYSTEM_PROMPT = (
    ...
    "Exception: if currency is not printed explicitly, infer it from strong "
    "contextual signals (seller/buyer country or address, decimal-comma vs "
    "decimal-point formatting) rather than returning null -- a competent "
    "bookkeeper would not leave it blank just because the symbol is missing. "
    "This inference allowance applies to currency only: every other field "
    "(amounts, dates, names, identifiers) must stay null when not clearly "
    "present, and field_confidence should be lower for an inferred value "
    "than for one read directly off the page. "
